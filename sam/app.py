...
def AutomateBatchDividing(year):
    ...
    query = "select s.studentid,s.cgpa from student s where s.studentid not in (select s.studentid from student s,batch b where s.batchno=b.batchid and b.batchtype = 'custom' and b.section=s.section and b.year=s.year) and s.section = %s and s.year=%s;"
...
def CustomDividing(year):
    if request.method == 'POST':
        ...
        getdeta = cursor.execute("select s.studentid,s.cgpa from student s where s.studentid not in (select s.studentid from student s,batch b where s.batchno=b.batchid and b.batchtype = 'custom' and s.year=b.year) and s.section = %s and s.year=%s;",(sec,year,))
        users = cursor.fetchall()
        print("users")
        print(users)
        connection.commit()
...
def GuideListing(year,sec):
    ...
    print(year)
    if sec == 'A' or sec == 'a':
        select_query = "SELECT guidename FROM guide where section_a is not null and year=%s"
        cursor.execute(select_query,(year,))
        users = cursor.fetchall()   
    elif sec == 'B' or sec == 'b':
        select_query = "SELECT guidename FROM guide where section_b is not null and year=%s"
        cursor.execute(select_query,(year,))
        users = cursor.fetchall()
    else:
        select_query = "SELECT guidename FROM guide where section_c is not null and year=%s"
        cursor.execute(select_query,(year,))
        users = cursor.fetchall()
...
def guideMapping():
    ...
    if column_name=='a':
        ...
        cursor.execute("UPDATE guide SET batch_1 = COALESCE((SELECT batchid FROM batch WHERE section='a' and guidename = guide.guidename AND batchtype = 'custom' and year=guide.year and year = %s LIMIT 1), 0),batch_2 = COALESCE((SELECT batchid FROM batch WHERE section='a' and guidename = guide.guidename  and  batchtype = 'custom' and year=guide.year and year=%s LIMIT 1 OFFSET 1), 0) WHERE guide.section_a IS NOT NULL and guide.year=%s;",(year,year,year,))

        
    elif column_name=='b':
        #cursor.execute("UPDATE guide SET  batch_1 = COALESCE((SELECT batchid FROM batch WHERE guidename = guide.guidename and section = guide.section_a AND batchtype = 'custom' LIMIT 1), 0),batch_2 = COALESCE((SELECT batchid FROM batch WHERE guidename = guide.guidename AND batchtype = 'custom' LIMIT 1 OFFSET 1), 0);")
        cursor.execute("UPDATE guide SET batch_1 = COALESCE((SELECT batchid FROM batch WHERE section='b' and guidename = guide.guidename AND batchtype = 'custom' and year=guide.year and year=%s LIMIT 1), 0),batch_2 = COALESCE((SELECT batchid FROM batch WHERE section='b' and guidename = guide.guidename  and  batchtype = 'custom' and year=guide.year and year=%s LIMIT 1 OFFSET 1), 0) WHERE guide.section_b IS NOT NULL and guide.year=%s;",(year,year,year,))

    else:
        cursor.execute("UPDATE guide SET batch_1 = COALESCE((SELECT batchid FROM batch WHERE section='c' and guidename = guide.guidename AND batchtype = 'custom' and year=guide.year and year = %s LIMIT 1), 0),batch_2 = COALESCE((SELECT batchid FROM batch WHERE section='c' and guidename = guide.guidename  and  batchtype = 'custom' and year = guide.year and year=%s LIMIT 1 OFFSET 1), 0) WHERE guide.section_c IS NOT NULL and guide.year=%s;",(year,year,year,))
    ...
    if column_name == 'a':
        if column_name:
            query = "select guidename,domains from guide where section_a is not null and year=%s"
            ...
            print("")
            print(guides)
            avail = cursor.execute(" select guidename , case when batch_1 =0 and batch_2 =0 then 2 when batch_1=0 or batch_2 =0 then 1 else 0  end as cnt from guide where section_a is not null and year=%s group by guidename;",(year,))
            avail_guides = cursor.fetchall()
            connection.commit()
            ava = dict(avail_guides)
    elif column_name == 'b':
        if column_name:
            query = "select guidename,domains from guide where section_b is not null and year=%s"
            guide_query = cursor.execute(query,(year,))
            guide = cursor.fetchall()
            connection.commit()
            row_headers1=[x[0] for x in cursor.description] #this will extract row headers
            ...
            print("")
            print(guides)
            avail = cursor.execute(" select guidename , case when batch_1 =0 and batch_2 =0 then 2 when batch_1=0 or batch_2 =0 then 1 else 0  end as cnt from guide where section_b is not null and year=%s group by guidename;",(year,))
            avail_guides = cursor.fetchall()
            connection.commit()
            ava = dict(avail_guides)
    else:
        if column_name:
            query = "select guidename,domains from guide where section_c is not null and year=%s"
            guide_query = cursor.execute(query,(year,))
            guide = cursor.fetchall()
            connection.commit()
            row_headers1=[x[0] for x in cursor.description] #this will extract row headers
            ...
            print("")
            print(guides)
            avail = cursor.execute(" select guidename , case when batch_1 =0 and batch_2 =0 then 2 when batch_1=0 or batch_2 =0 then 1 else 0  end as cnt from guide where section_c is not null and year = %s group by guidename;",(year,))
            avail_guides = cursor.fetchall()
            connection.commit()
            ava = dict(avail_guides)
...
def get_student_data(student,year):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("update batch join (select *,sum(btc)/count(batchno)as cnt from(select *,sum(ov) / count(batchno) as btc from(select name,batchno,section,reviewno,sum(Overall div 2) as ov from rev  group by name,reviewno,batchno)sb group by batchno,reviewno)tr group by batchno)t2 on batch.batchid=t2.batchno set batch.review_score=t2.cnt where t2.section=batch.section;")
    conn.commit()
...
def generate():
    ...
    print(username)
    year = request.json.get('year')
    user1 = get_student_data(username,year)
    print(user1)
