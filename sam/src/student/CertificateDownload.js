...
/*
  ...
  useEffect(() => {
    const fetchTopRankStudents = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:5000/top-rank-students/${year}`);
  ...
  return (
    <div className=''>
      <h2 style={{marginLeft:'20px'}}>Certificate Download</h2>
      {isTopRank ? (
        <>
        <button onClick={handleDownload} style={{marginLeft:'20px'}}>Download Certificate</button>
        {pdfUrl && <iframe src={pdfUrl} style={{ width: '100%', height: '500px' }}></iframe>}
      </>
      ) : (
        <p style={{marginLeft:'20px'}}>You are not in the top-rank list.</p>
      )}
    </div>
  );
};
